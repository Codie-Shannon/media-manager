...
namespace Media_Manager.Views
{
    public partial class PicturesView : UserControl
    {
        ...
        private readonly string str_Exists = "The picture has already been added to the database. Are you sure you want continue with this procedure?";
        ...
        // =====================================================================
        // =====================================================================
        public PicturesView() { InitializeComponent(); }

        public PicturesView(int id, int[] indexes = null, Stack<Stack<Tuple<int, int, string, string>>> navigationunloadedstack = null, Stack<Stack<Tuple<int, int, string, string>>> navigationloadedstack = null, int activefolder = 0, bool isfavourite = false)
        {
        ...
        private void Page_Unloaded(object sender, RoutedEventArgs e)
        {
            ...
            CoverImages.Clean(Properties.Settings.Default.Pictures);
        }
        ...
        private void ItemsControl_MouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            if (e.OriginalSource is ScrollViewer && isSelectionChanged == false)
            {
                ...
                Elements.UnsetElements(ref icItems, ref selectedid, ref selectedPicture, ref selectedFolder, ref selectedType);
        ...
        {
            //Update Favourite
            UpdateFavourite(selectedPicture.isFavourite == 1 ? 0 : 1);
        }
        ...
        private void OpenElementPanel_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                //Load Add Item
                OpenPanel("Add", "Picture", btnProcessItem, ItemPanel);
            }
        ...
        private void odPicturePath_Click(object sender, RoutedEventArgs e)
        {
            //Get Options Panel (Owner)
            optPanel owner = (optPanel)OptionsPanel.GetAncestor((FrameworkElement)sender, typeof(optPanel));
        ...
        private void ShowInExplorer_Click(object sender, RoutedEventArgs e)
        {
            ...
            Elements.ShowInExplorer(FileType.File, selectedPicture.FilePath);
        }
        ...
        private async void NavigateToFolder(int id)
        {
            ...
            Elements.SetElement(ref icItems, id, out selectedid, Pictures.SingleOrDefault(i => i.Id == id), out selectedPicture, Folders.SingleOrDefault(i => i.Id == id), out selectedFolder, ElementType.Folders, out selectedType);
        ...
        private async void LoadElementContextMenu(object sender)
        {
            ...
            if (selectedType == ElementType.Files)
            {
                ...
                int isfavourite = selectedPicture.isFavourite;
        ...
        private void BringInToView()
        {
            ...
            if (element == null || !element.IsVisible)
            {
                ...
                vwpItems.BringIndexIntoViewPublic(selectedType == ElementType.Files ? Composite.IndexOf(selectedPicture) : Composite.IndexOf(selectedFolder));
            }
        }
        ...
        private async Task ItemSelectedAsync(int id)
        {
            ...
            if (selectedId != 0 && selectedPicture != null && selectedId == id)
            {
                //Check if File is Valid
                if (Validation.File(selectedPicture.FilePath))
                {
                    //Get The Main Window
                    Window mainWindow = Application.Current.MainWindow;

                    //Get Active Pictures from Pictures ObservableCollection
                    List<Picture> pictures = tbtnShowFavourites.IsSelected ? Pictures.Where(i => i.OwnerId == activeFolder && i.isFavourite == 1).ToList() : Pictures.Where(i => i.OwnerId == activeFolder).ToList();

                    //Set Main Window's Data Context to New Picture Gallery View
                    mainWindow.DataContext = new PictureGalleryView(cboxSortBy.GetIndexes(), nbNavigationBar.GetStack(NavigationBarStackType.Unloaded), nbNavigationBar.GetStack(NavigationBarStackType.Loaded), activeFolder, tbtnShowFavourites.IsSelected, pictures, selectedPicture);
                }
                else
                {
                    //Show Error Message
            ...
            else
            {
                ...
                //Wait for the Items to be added to the Pictures Observable Collection (Required for Return)
                await Task.Delay(300);

                //Set Selected Element
                Elements.SetElement(ref icItems, id, out selectedid, Pictures.SingleOrDefault(i => i.Id == id), out selectedPicture, Folders.SingleOrDefault(i => i.Id == id), out selectedFolder, ElementType.Files, out selectedType);
        ...
        {
            //Display Main
            Title.Content = Formatter.FormatName(selectedPicture.Name, selectedPicture.CustomName);
            Cover.Source = Formatter.FormatImage(MediaType.Pictures, selectedPicture.CoverImage);

            //Display Standard Details
            tbResolution.Content = Formatter.FormatResolution(selectedPicture.Width, selectedPicture.Height);
            tbFormat.Content = selectedPicture.Format;
            tbFileSize.Content = Formatter.FormatFileSize(selectedPicture.FileSize);
            tbCreated.Content = Formatter.FormatCreation(selectedPicture.CreationTime, selectedPicture.CreationDate);

            //Display Advance Details
            tbColourSpace.Content = selectedPicture.ColourSpace;
            tbBitDepth.Content = Formatter.FormatBitDepth(selectedPicture.BitDepth);
            tbCompMode.Content = selectedPicture.CompMode;
        }

        private async void UpdateFavourite(int value)
        {
            //Set selectedPicture isFavourite Value
            selectedPicture.isFavourite = value;

            //Set Pictures Observable Collection's selectedPicture isFavourite Value
            Pictures.ElementAt(Pictures.IndexOf(selectedPicture)).isFavourite = value;

            //Update selectedPicture's Database isFavourite Value
            Database.UpdateFavourite(selectedPicture.Id, value, MediaType.Pictures);

            //Check if Favourites are being Shown at the moment and the selectedPicture is being Unfavourited
        ...
        private void ClearPanel(string name)
        {
            ...
            if (name == "itempanel")
            {
                ...
                OptionsPanel.ClearPanel(null, new List<optTextBoxLong> { tbCustomName }, null, new List<optOpenDialog> { odPicturePath });
            }
        ...
        private async void AddFolder()
        {
            ...
            Folder folder = new Folder() { OwnerId = fbdFolderSaveLocation.Id, Name = $"{tbFolderName.Content}", Type = nameof(MediaType.Pictures), FolderType = nameof(FolderType.Folders) };
        ...
        private void AddItem()
        {
            ...
            //Get Base Item Data
            Picture picture = new Picture() { OwnerId = fbdItemSaveLocation.Id, FilePath = $"{odPicturePath.Content}", CustomName = $"{tbCustomName.Content}" };
            ...
            //Get Boolean Value Representing if the Item Already Exists within the Database (Pictures ObservableCollection)
            bool isEmpty = !Pictures.Any(i => picture.FilePath == i.FilePath);

            //Validate File Path
            bool isValid = Validation.Process(picture.FilePath);
            ...
            if (isAddItem)
            {
                ...
                ProcessAddItemAsync(picture);
            }
        }

        private async void ProcessAddItemAsync(Picture picture)
        {
            //Show Loading Panel
            ToggleState.Loading(true);

            //Configure Fetcher
            Fetcher.ConfigureFetcherAsync(MediaType.Null, picture.FilePath);

            //Get Item Details
            picture = Fetcher.GetPicture(Pictures.Count() + 1, picture);

            //Save Item
            Database.SaveItem(MediaType.Pictures, picture);
        ...
        private void LoadItemEdit()
        {
            ...
            if (Pictures.Contains(selectedPicture) && Validation.File(selectedPicture.FilePath))
            {
                //Set Panel
                SetPanel("Edit", "Picture");

                //Load Editable Values
                OptionsPanel.LoadEditableValues(tbCustomName, selectedPicture.CustomName, fbdItemSaveLocation, selectedPicture.OwnerId, nbNavigationBar.GetFolderPath(), selectedType, selectedid, new List<optOpenDialog> { odPicturePath }, new List<string> { selectedPicture.FilePath });
        ...
        {
            //Get Base Item Data
            Picture picture = new Picture() { FilePath = $"{odPicturePath.Content}", CustomName = $"{tbCustomName.Content}", OwnerId = fbdItemSaveLocation.Id };

            //Hide Item Panel
            ToggleState.Panel(ItemPanel, false);

            //Validate File Path
            if (Validation.ValidateSaveLocation(OptionPanelType.Null, Folders.ToList(), new KeyValuePair<int, string>(picture.OwnerId, fbdItemSaveLocation.FolderPath)) && Validation.Process(picture.FilePath))
            {
                //Show Loading Panel
                ToggleState.Loading(true);

                //Check if the file path has been changed
                if (picture.FilePath != selectedPicture.FilePath)
                {
                    //Configure Fetcher
                    Fetcher.ConfigureFetcherAsync(MediaType.Null, picture.FilePath);

                    //Get Picture Details
                    picture = Fetcher.GetPicture(selectedPicture.Id, picture);
                }
                else
                {
                    //Assign selectedPicture into picture object
                    picture = selectedPicture;

                    //Update Picture Values
                    picture.CustomName = tbCustomName.Content.ToString();
                    picture.OwnerId = fbdItemSaveLocation.Id;
                }

                //Update Item
                Database.UpdateItem(selectedPicture.Id, picture, MediaType.Pictures);
        ...
        private async void RemoveFolder()
        {
            ...
            bool isRemoved = DiscardElements.RemoveFolder(MediaType.Pictures, selectedId);
        ...
        private async void RemoveItem(RemovalType type = RemovalType.Remove)
        {
            ...
            if (type == RemovalType.Delete && Validation.File(selectedPicture.FilePath))
            {
                //Delete the Item from the Computer System
                DiscardElements.DeleteItem(out isCancel, out isDeleted, MediaType.Pictures, FileType.File, selectedPicture.FilePath);
            }
            ...
            if (isDeleted == true || type == RemovalType.Remove)
            {
                ...
                bool isRemoved = DiscardElements.RemoveItem(MediaType.Pictures, selectedPicture.Id, type, selectedPicture.CoverImage);
        ...
        private async Task LoadAsync(ElementType elementtype = ElementType.Null, Folder folder = null)
        {
            ...
            if (Pictures.Count() > 0) { Composite.AddRange(tbtnShowFavourites.IsSelected ? Pictures.Where(i => i.OwnerId == activeFolder && i.isFavourite == 1) : Pictures.Where(i => i.OwnerId == activeFolder)); }
        }

        private async Task LoadItemsAsync()
        {
            //Clear Pictures Observable Collection
            Pictures.Clear();
            ...
            await Task.Run(() =>
            {
                //Loop Through Each Picture in the Pictures Database
                foreach (Picture picture in Database.LoadItems(MediaType.Pictures, order, type))
                {
                    //Invoke Dispatcher
                    Dispatcher.Invoke(() =>
                    {
                        //Add Each Picture from the Pictures Database to the Pictures Observable Collection
        ...
        private async Task LoadFoldersAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                foreach (Folder folder in Database.LoadFolders(MediaType.Pictures))
                {
                    //Invoke Dispatcher
                    Dispatcher.Invoke(() =>
                    {
        ...
        private void InputValidation(string owner)
        {
            ...
            if (owner == "itempanel" && isEdit && selectedPicture != null)
            {
                //Validate Panel Contents
                Validation.ValidateGenericPanel(OptionPanelType.Edit, btnProcessItem, Folders.ToList(), new string[] { selectedPicture.CustomName, selectedPicture.FilePath }, false, 0, new KeyValuePair<int, string>(fbdItemSaveLocation.Id, fbdItemSaveLocation.FolderPath), new string[] { $"{tbCustomName.Content}", $"{odPicturePath.Content}" }, new KeyValuePair<int, string>(selectedPicture.OwnerId, nbNavigationBar.GetFolderPath()));
            }
            else if (owner == "itempanel" && !isEdit)
            {
                //Validate Panel Contents
                Validation.ValidateGenericPanel(OptionPanelType.Add, btnProcessItem, Folders.ToList(), new string[] { $"{odPicturePath.Content}" }, false, 0, new KeyValuePair<int, string>(fbdItemSaveLocation.Id, fbdItemSaveLocation.FolderPath));
            }
